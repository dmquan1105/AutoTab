...
class WindowRenderer:
    ...
    def render(self, workbook: str | Path, sheet: str, cell_range: str, output: Path, metadata: dict) -> dict:
        try:
        ...
        metadata.update({
            "sheet": sheet, "range": cell_range, "image_width": width,
            "image_height": height, "renderer": renderer,
...
def _render_with_libreoffice(workbook: str | Path, sheet: str, cell_range: str, output: Path) -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="autotab_render_") as temp:
        ...
        try:
            ...
            ws.page_margins.left = ws.page_margins.right = ws.page_margins.top = ws.page_margins.bottom = 0
            ...
            wb.close()
        command = [str(soffice), "--headless", "--nologo", "--nofirststartwizard", "--nodefault", "--nolockcheck", f"-env:UserInstallation={profile.resolve().as_uri()}", "--convert-to", "pdf:calc_pdf_Export", "--outdir", str(temp_dir), str(prepared)]
...
def _prepare_dimensions(ws: object, cell_range: str) -> None:
    ...
    for row in ws.iter_rows(min_row=min_row, max_row=max_row, min_col=min_col, max_col=max_col):
        for cell in row:
            ...
            columns = range(merged.min_col, merged.max_col + 1) if merged else range(cell.column, cell.column + 1)
            available = sum(float(ws.column_dimensions[get_column_letter(col)].width or 13) for col in columns)
            ...
            if cell.value is not None and (cell.alignment.wrap_text or "\n" in str(cell.value)):
                ...
                line_count = sum(max(1, (len(line) + max(1, int(available)) - 1) // max(1, int(available))) for line in lines)
                ws.row_dimensions[cell.row].height = max(
                    float(ws.row_dimensions[cell.row].height or 18), line_count * float(cell.font.sz or 11) * 1.25 + 4
                )
...
def _render_with_pillow(workbook: str | Path, sheet: str, cell_range: str, output: Path) -> str:
    ...
    widths = [max(90, min(360, max([_display_width(str(row[i] or "")) for row in values] + [8]) * 8 + 24)) for i in range(max_col - min_col + 1)]
    row_height = 30
    ...
    for row_index, row in enumerate(values, min_row):
        ...
        for i, value in enumerate(row):
            ...
            color = f"#{fill[-6:]}" if fill and len(fill) >= 6 else ("#eaf2f8" if row_index == min_row else "white")
            draw.rectangle((x, y, x + widths[i], y + row_height), fill=color, outline="#b7c3d0")
            draw.text((x + 8, y + 7), str(value or "")[: max(1, widths[i] // 8 - 2)], fill="#17202a")
            x += widths[i]
        y += row_height
    output.parent.mkdir(parents=True, exist_ok=True)
    image.save(output)
...
def _trim_image(path: Path) -> None:
    with Image.open(path) as image:
        ...
        if bbox:
            rgb.crop((max(0, bbox[0] - 6), max(0, bbox[1] - 6), min(rgb.width, bbox[2] + 6), min(rgb.height, bbox[3] + 6))).save(path)
