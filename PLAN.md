...
Implement only the Exploration phase first. Given `.xlsx` workbooks and a natural-language query, produce a grounded `exploration.md` containing relevant keywords, source ranges, descriptions, and relations. QA, agent execution, operators, Observe, and validation loops are deferred.
...
The final output is Markdown. Descriptions and relations must be natural-language statements grounded in the visible cells. Every statement must resolve to the source range rendered for that keyword.
...
Add a versioned structured LLM prompt for entities, metrics, dimensions, time expressions, filters, and operations. Normalize case/whitespace, preserve meaningful phrases, deduplicate aliases, and retain source spans. Provide deterministic phrase/token fallback when the model is unavailable.
...
2. **Semantic search** embeds the keyword (including aliases) and every `CellDocument`, then ranks by cosine similarity.
...
```text
viewports/<finding_id>/image.png
viewports/<finding_id>/cells.json
viewports/<finding_id>/metadata.json
```
...
By default, send only the keyword/aliases, image, and cell-finding context to the VLM. Sending the original user query is configurable and defaults to disabled to reduce prompt leakage and keep viewport analysis focused. When enabled, include the query explicitly and record that choice in the manifest. The keyword and source range already belong to the cell finding and must be attached by the pipeline, not regenerated by the VLM. Ask the VLM to return only a natural-language description and natural-language relations about the visible viewport. Cache successful responses by input/model/prompt hashes. Validate the returned text and grounding against `cells.json`; retry one malformed response with the validation error, then continue with a typed warning.
...
- range: `Data_Synthesis!A1:I9`
- description: The image contains monthly revenue columns.
- relations:
  - Revenue is grouped by region in `Data_Synthesis!A5:D7`.
```

Include run ID, query, workbook hashes, config snapshot, and model/prompt versions. Order keywords by query order, then evidence by relevance and source location. Repeated inputs must produce identical Markdown apart from explicitly marked timestamps.
...
- range: `Data_Synthesis!A1:I9`
- description: The sheet shows revenue values organized by month and region.
- relations:
  - Revenue is grouped by region.
  - Monthly revenue columns contribute to the quarterly total.

## region

- range: `Data_Synthesis!A5:A7`
- description: The range lists the regions represented in the table.
- relations:
  - Each region has corresponding revenue values in the adjacent columns.
```

`description` and every relation are complete natural-language sentences. Ranges use workbook-qualified sheet notation, and every statement must be supported by rendered cells. Omit a keyword section when no hit reaches the threshold; record that omission in the manifest.
